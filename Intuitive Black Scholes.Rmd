---
title: "Intuitive Probabalistic Derivation of Black Scholes"
output: html_notebook
---
Today I will be debuting my first mathematics blog post. Hopefuly it will be part of my series on all the creative derivations of Black Scholes Merton Option Pricing Model (BSM-OPM). This is the most simple one I've found, that uses only logic of compounding prices, and Log Normal Distributions. 

**Lets Start by defining features of Stock Prices and Random Variables:**

1. Future Value of Stock Price is the price today compounded continuosly into the future $F_0^T = S_0e^{rT}$. If you want more info Check out version of Hull in the chapter titled "Properties of Stocl Options".

2. Random Variables of a log normal distribution $x_L$:

    a. continuously distributed over $0<x_L<+\infty$.
  
    b. follows probability density function $f_L(x_L)$.
  
    c. The standard normal random variable $x_N=ln[x_L]$.
  
    d. Define conventions for means and variances of these distributions:
  
        i. Log Normal Distribution has Mean and Variance $\mu_L$ and $\sigma^2_L$
    
        ii. Standard Normal Distribution has Mean and Variance $\mu_N$ and  $\sigma^2_N$.
    
3. We can caluclate the log normal mean $\mu_L$:
 
      $\mu_L= \int_0^\infty{x_Lf_L(x_L)}dx_L$
  
      $\mu_L= e^{\mu_N + \frac{1}2\sigma^2_N}$

4. Now lets introduce a 'tructated' also called partial distribution of $x_L$ which we will call $\mu_k$, where $k<x_L<+\infty$ and $k>0$. 
...
**Justification of Black Scholes:**

1. Lets define some things:
